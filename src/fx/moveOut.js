...
const moveOut = (element, toDirection = 'right', completeFn, settings) => {
    if (!element) {
    ...
    if (
        ...
        self.setTimeout(
            () => applyTransition(
                ...
                config,
                {
                    'transform': self.getComputedStyle(element).getPropertyValue('transform').replace(matrixRegex, `matrix($1, $2, $3, $4, ${ (transformValues && transformValues[6]) || (direction === 'left' || direction === 'right') ? amount : 0}, ${ (transformValues && transformValues[5]) || (direction === 'top' || direction === 'bottom') ? amount: 0})`),
                },
                () => {
